...
class CryptoCurrencyDataset(Dataset):
    def __init__(self, dataframe, sequence_length=16):
        ...
        self.y = torch.tensor(self.dataframe[target_columns].values,dtype=torch.long).squeeze()
...
def create_dataloaders(data_preprocessor: DataPreprocessor, sequence_length: int=8, batch_size: int=16):
    train_dataset = CryptoCurrencyDataset(data_preprocessor.train_data, sequence_length=sequence_length)
    validation_dataset = CryptoCurrencyDataset(data_preprocessor.validation_data, sequence_length=sequence_length)
    test_dataset = CryptoCurrencyDataset(data_preprocessor.test_data, sequence_length=sequence_length)   
